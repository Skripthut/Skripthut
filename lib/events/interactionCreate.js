...
module.exports = async function(interaction) {
	if (interaction.isCommand()) {
		...
		**/
		...
		let logs = guild.channels.cache.get(database.guilds?.[guild.id]?.message_logs);
		...
		*/
		...
		if (command === 'createticket') {
			...
			database.ticketMessages[sentMessage.id] = {
				id: sentMessage.id,
		...
		// CLOSE COMMAND
		if (command === 'close') {
			let ticket = database.tickets?.[channelId];
			if (ticket) {
				...
				database.tickets[channelId].closed = true;
		...
		if (command === 'reactionrole') {
			...
			if (subcommand === 'create') {
				...
				reactionRoleMessage.then(async (message) => {
					...
					database.reactionRoleMessages[message.id] = {
						id: message.id,
						emotes: {}
					}
					database.reactionRoleMessages[message.id].emotes[reactionRoleEmoteId] = new ReactionRoleEmote(message, reactionRole);
				});
				return;
			}

			// "add" and "remove" subcommands
			let ids = Array.from(args.message.matchAll(/https:\/\/database\.com\/channels\/(\d+)\/(\d+)\/(\d+)/))[0]; // Get ID's from URL
			...
			if (type === 'add') {
				...
				_reactionRoleMessage = database.reactionRoleMessages?.[reactionRoleMessageId];
				...
				if (!_reactionRoleMessage) {
					_.set(database, `reactionRoleMessages.${reactionRoleMessageId}`, 
					{
						id: reactionRoleMessageId,
						emotes: {}
					});
				}
				database.reactionRoleMessages[reactionRoleMessageId].emotes[reactionRoleEmoteId] = new ReactionRoleEmote(reactionRoleMessage, reactionRole);
				return;
			}
			...
			if (type === 'remove') {
				...
				**/
				function removeEmotes(emotes) {
					...
					for (let [key, value] of entries) {
						if (value.role === reactionRoleId) {
							delete database.reactionRoleMessages[reactionRoleMessageId].emotes[key];
				...
				if (!removeEmotesResult.length) {
					delete database.reactionRoleMessages[reactionRoleMessageId];
				}
		...
		*/
		if (command === 'ban') {
			...
			if (details) {
				...
				_.set(database.guilds, `${guildId}.members.${target.id}.banned`, {
					banDate: details.now,
