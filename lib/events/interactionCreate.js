...
const SkriptSyntax = require(`../util/SkriptSyntax.js`);
	...
	if (interaction.isCommand()) {
		...
		*/
		...
		if (command === 'addon') {
			...
			let addon = await SkriptSyntax.getAddon(addonName, api);
			...
			if (api === 'skripttools') {
				...
				addonInfo = SkriptSyntax.getAddonInfo(file, api);
			}
			else if (api === 'skripthub') {
				addonInfo = SkriptSyntax.getAddonInfo(addon, api);
			}
		...
		if (command === 'docs') {
			...
			/** @type {string} The docs API to use */
			let api = args.api.toLowerCase();

			let syntaxResult = await SkriptSyntax.searchForSyntax(query, api);
			...
			const apiName = { skunity: 'skUnity Docs 2', skripthub: 'SkriptHub Docs 1' }[api];
		...
		*/
		...
		if (command === 'role') {
			...
			if (subcommand === 'grant') {
				...
				r.reply(`Added ${role} to ${target}'s roles!`, "followUp");
				return;
			}
			if (subcommand === 'revoke') {
				...
				r.reply(`Removed ${role} from ${target}'s roles...`, "editReply");
				...
				if (color.match(/[a-f0-9]{6}/i)) {
					...
					guild.roles.create(
					{
						...
						.then(async (result) => {
							...
							r.reply(`Added ${result} to your roles!`, "followUp");
						});
					return;
				}
		...
		*/
		...
		if (command === 'mute') {
			...
			if (args.timespan) {
				...
				setStat(targetMember, "banDate", details.date);
