...
const { getMetadata } = require(`../methods/metadata.js`);
const { getStat } = require(`../methods/stats.js`);
...
module.exports = async function(reaction, user) {
	...
	//console.log('message', message, guild, message.guildId);
	...
	const ticket = database.ticketMessages?.[message.id];
	if (ticket) {
		await message.reactions.resolve("📰").users.remove(user.id);
		if (getStat(member, "hasTicket")) { return; }
		database.totalTickets++;
		var ticketName = `Ticket-${database.totalTickets}`;
		guild.channels.create(ticketName, {
			...
			.then(async (channel) => {
				...
				const embed = new Discord.MessageEmbed()
					.setColor('#0099ff')
					.setTitle(ticketName)
					.setURL(`https://database.com/channels/${guild.id}/${message.channel.id}/${message.id}`)
				...
				_.set(database, `tickets.${channel.id}`, {
					id: channel.id,
					name: ticketName,
					member: user.id,
					message: message.id
				...
				setTimeout(async function() {
					...
					channel.send(embed).then((message) => {
						...
						embed.url = `https://database.com/channels/${guild.id}/${channel.id}/${message.id}`;
	...
	const reactionRole = database.reactionRoleMessages?.[message.id];
	if (reactionRole) {
