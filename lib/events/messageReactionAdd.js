...
module.exports = async function(reaction, user) {
	...
	const ticket = database.discord.ticketMessages?.[message.id];
	if (ticket) {
		await message.reactions.resolve("📰").users.remove(user.id);
		if (getStat(member, "hasTicket")) { return; }
		database.discord.totalTickets++;
		var ticketName = `Ticket-${database.discord.totalTickets}`;
		guild.channels.create(ticketName, {
			...
			.then(async (channel) => {
				...
				_.set(database.discord, `tickets.${channel.id}`, {
					id: channel.id,
					name: ticketName,
					member: user.id,
					message: message.id
