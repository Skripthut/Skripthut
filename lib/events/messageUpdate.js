...
module.exports = async function(oldMessage, newMessage) {
	...
	if (logs) {
		...
		const embed = new Discord.MessageEmbed()
			...
			.addFields(
				{ name: 'Old Message', value: oldContent.substring(0, Math.min(oldContent.length, 2000 - descLength)) },
				{ name: 'New Message', value: newContent.substring(0, Math.min(newContent.length, 2000 - descLength)) }
			);
		logs.send(embed);
	}
}