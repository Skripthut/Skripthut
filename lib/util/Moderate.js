...
module.exports = class Moderate {
    ...
    **/
    static async getPunishmentDetails(millisTimespan) {
        ...
        return {
            now: now, // The unix starting date of the punishment
            milliseconds: milliseconds, // The length of the punishment in milliseconds
            readableTimespan: time.join(", "), // The readable timespan of the punishment
            endDate: (milliseconds) ? new Date(now + milliseconds) : null // The Date the punishment will end
        };
    }
}